...
| QG-065 | README startup works on a clean clone. | PASS | GitHub Actions checked out exact pushed SHA `988a676`, installed under Node 24 with `npm ci`, passed `npm audit`, typecheck, lint, all 90 Vitest checks, a production build, and all 18 Playwright flows—including startup through the documented production command. |
...
- The clean [GitHub Actions run](https://github.com/solahai/referralarc/actions/runs/33342229710) passed install, dependency audit, typecheck, lint, Vitest, build, browser installation, and the complete Playwright suite on exact pushed SHA `988a676`.
