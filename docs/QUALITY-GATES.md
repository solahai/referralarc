...
Status vocabulary is closed: `PASS` means current repository or exact public-release evidence directly satisfies the gate; `WATCH` means an ongoing release obligation or partial evidence remains; `MANUAL` means a human or real-agent check is inherently required; `BLOCKED` means an external condition currently prevents completion.

| ID | gate | status | evidence/next action |
|---|---|---|---|
| QG-001 | Production build passes. | PASS | A clean Node 24 production build of application source `c2a4482` completed successfully before Sites version 9 was deployed on 2026-08-31. |
| QG-002 | TypeScript passes with no errors. | PASS | `npm run typecheck` completed with exit 0 on the post-deployment working tree on 2026-08-31. |
| QG-003 | Lint passes. | PASS | `npm run lint` completed with exit 0 on the post-deployment working tree on 2026-08-31. |
| QG-004 | Unit tests pass. | PASS | `npm test` completed 90 of 90 Vitest checks across six files on the post-deployment working tree on 2026-08-31; unit coverage includes 29 checks in `tests/unit/engine.test.ts`. |
...
| QG-006 | E2E tests pass. | PASS | All 18 Playwright checks passed against the exact public production URL on 2026-08-31. |
| QG-007 | Accessibility tests pass. | PASS | The public production run passed Axe checks for landing, initial demo, prepared review, approved, and receipt states. |
| QG-008 | No serious/critical axe violations. | PASS | The public production E2E run reported zero serious or critical Axe findings in every scanned state. |
| QG-009 | No console errors on golden flow. | PASS | The public agent + human interleaving test attached a console-error listener before loading and kept it empty through the final receipt. |
| QG-010 | No unhandled promise rejections. | PASS | The public golden test kept its `pageerror` listener empty through the final receipt; registration and clipboard failure paths also passed. |
...
| QG-026 | Writes produce human-readable receipt. | PASS | The public production E2E flow reached the visible receipt card, and `docs/screenshots/receipt-1440x900.jpg` was regenerated from the same deployed application source. |
...
| QG-033 | Dynamic tool state is visible in UI. | PASS | Public E2E and native Chrome runs verified nine initial tools, `prepare_booking` absent then present after selection, and `commit_booking` absent then present after approval; the boundary derives its headline from successful registrations only. |
| QG-034 | Tool activity is visible in UI. | PASS | The public golden E2E explicitly verifies the activity title, commit kind, localized timestamp, duration, tool name, receipt identifier, and second-resolution registration event; the rail also renders summary and changed fields. |
| QG-035 | Golden demo resets in one click. | PASS | The public Playwright run exercised the single `Reset demo` control in native-surface, fallback, and navigation-persistence flows. |
...
| QG-043 | Unsupported browser receives useful fallback. | PASS | The public production E2E run completed the no-WebMCP human flow through confirmation and receipt and verified truthful fallback copy. |
...
| QG-047 | Mobile width 390px has no horizontal overflow. | PASS | The public production E2E run verified root width at 390px plus prepared and approved internals at 320px and all three decision buttons at 320, 600, and 700px. |
...
| QG-052 | Screen-reader labels exist for controls. | PASS | Public E2E checks every rendered button and link for a nonempty accessible name, verifies the appointment-slot combobox label, and passes Axe in prepared, approved, and receipt states. |
...
| QG-060 | Agent activity timestamps render correctly. | PASS | The public golden E2E verifies localized activity time to the minute and registration time to the second, plus measured duration in milliseconds. |
| QG-061 | Audit state survives ordinary navigation. | PASS | The public Playwright run verified demo-to-landing-to-demo persistence, a fresh server response, nested-route reload, new-tab isolation, and explicit reset. |
...
| QG-065 | README startup works on a clean clone. | PASS | [GitHub Actions](https://github.com/solahai/referralarc/actions/runs/33342453726) checked out deployed source SHA `c2a4482`, installed under Node 24 with `npm ci`, passed `npm audit`, typecheck, lint, all 90 Vitest checks, a production build, and all 18 Playwright flows—including startup through the documented production command. |
...
| QG-070 | Submission claims match what the product demonstrably does. | PASS | `docs/SUBMISSION.md` is reconciled to Sites version 9, avoids outcome and integration overclaims, and keeps video, Devpost, and entrant attestations explicitly pending. |
| QG-071 | Public deployment loads from a clean browser. | PASS | Sites version 9 returns HTTP 200 without login at the public URL and passed all 18 Playwright checks in isolated browser contexts. |
| QG-072 | Public deployment deep-link works. | PASS | Direct navigation to `https://referralarc.docsplainai.chatgpt.site/demo` returns HTTP 200, hydrates, and passed the full public E2E suite. |
| QG-073 | Public deployment refresh on nested route works. | PASS | The public E2E suite reloads `/demo` in a fresh page and verifies the hydrated initial workflow after the direct server response. |
| QG-074 | CSP/security headers do not break WebMCP. | PASS | The public response carries CSP, Permissions Policy, Origin-Agent-Cluster, referrer, and MIME protections; native Chrome 151 completed the full lifecycle against that HTTPS origin. |
| QG-075 | Required origin isolation configuration is verified against current docs. | PASS | Current W3C and Chrome material was checked, and the public response returns `Origin-Agent-Cluster: ?1`; no `document.domain` use or cross-origin tool exposure exists. |
...
| QG-089 | README screenshots match final UI. | PASS | Both README images were regenerated in native Chrome 151 from the deployed application source after the last code and CSS changes; the public OG asset is byte-identical to the repository copy. |
...
- Keep deployed Sites version 9 free and unrestricted through September 21, 2026 at 5:00 p.m. PT.
...
- On 2026-08-31, typecheck, lint, all 90 Vitest checks, and all 18 enhanced Playwright checks passed; the browser suite ran against the exact public production URL.
- The clean [GitHub Actions run](https://github.com/solahai/referralarc/actions/runs/33342453726) passed install, dependency audit, typecheck, lint, Vitest, build, browser installation, and the complete Playwright suite on deployed source SHA `c2a4482`.
- The public Chrome 151 smoke recorded at 2026-08-31T01:15:15Z starts with nine tools, proves `prepare_booking` absent before selection and present afterward, proves `commit_booking` absent before approval and present afterward, consumes it once, finds the receipt, obtains both booking handles from structured output, leaves a pre-aborted write unchanged, and reconciles a late cancellation through structured state plus receipt. Show this native lifecycle in the final video.
...
- Exact-public CDP proxies—not Lighthouse scores—measured desktop landing LCP 708 ms / CLS 0 / TTFB 238 ms and slow-4G plus 4× CPU mobile-demo LCP 1.216 s / CLS 0 / TTFB 256 ms. A final Lighthouse run is optional evidence, not a challenge requirement.
- Exact production client chunks were scanned with no app/client secrets, source maps, host paths, or ReferralArc-owned development endpoints found.
- Tagging and freezing the final commit is a useful reproducibility control, not an Official Rules requirement by itself.
- Public OG, favicon, and third-party-notice bytes match the repository artifacts; native evidence records the exact production URL and Sites version 9.
